...
class PlanarARTracker:
    ...
    def blend_ad_into_roi(self, frame: np.ndarray, dst_pts: np.ndarray, feather_radius: int = 15) -> np.ndarray:
        """Blends the warped asset onto the video frame backdrop using feathered alpha blending."""
    ...
    def track_and_render_frame(self, img1: np.ndarray, frame_idx: int, log_file_handle) -> np.ndarray:
        ...
        # Blend graphic overlays onto the background image using feather blending
    ...
    def start_pipeline(self, video_path: str, target_width: int = 1280):
        ...
        output_dir = "./real-time-vid-tracking-and-enhancement/tracked_frames_feather"
        os.makedirs(output_dir, exist_ok=True)
        log_filepath = "./real-time-vid-tracking-and-enhancement/tracking_log_feather.txt"
        ...
        with open(log_filepath, "w", encoding="utf-8") as log_file:
            log_file.write("--- PLANAR TRACKING WITH FEATHER BLENDING RUN LOG STARTED ---\n")
            try:
                while True:
                    ret, frame = cap.read()
                    if not ret:
...
if __name__ == "__main__":
    ...
    try:
        # Pure feather blending for smooth alpha transitions
