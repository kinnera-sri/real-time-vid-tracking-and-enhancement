...
class PlanarARTrackerHarmonization:
    def __init__(self, ad_image_path: str, feature_limit: int = 2000):
        ...
        self.win_live = "Live Planar Tracking with Harmonization (Press 'q' to Quit)"
    ...
    def _apply_lab_harmonization(self, blended_img, frame, mask, strength=0.3):
        """Apply LAB color harmonization with saturation protection for vibrant elements."""
    ...
    def blend_ad_into_roi(self, frame: np.ndarray, dst_pts: np.ndarray, feather_radius: int = 15) -> np.ndarray:
        """Blends the warped asset onto the video frame backdrop using LAB harmonization."""
        ...
        # Alpha blend the warped ad with frame
        src_f = warped_ad.astype(np.float32)
        dst_f = frame.astype(np.float32)
        composite = (alpha_3c * src_f) + ((1.0 - alpha_3c) * dst_f)
        blended = np.clip(composite, 0, 255).astype(np.uint8)

        # Apply LAB harmonization for color matching
        harmonized = self._apply_lab_harmonization(blended, frame, mask_for_clone, strength=0.3)
    ...
    def track_and_render_frame(self, img1: np.ndarray, frame_idx: int, log_file_handle) -> np.ndarray:
        ...
        # Blend graphic overlays onto the background image using harmonization
    ...
    def start_pipeline(self, video_path: str, target_width: int = 1280):
        ...
        output_dir = "./real-time-vid-tracking-and-enhancement/tracked_frames_feather_harmonization"
        os.makedirs(output_dir, exist_ok=True)
        log_filepath = "./real-time-vid-tracking-and-enhancement/tracking_log_feather_harmonization.txt"
        ...
        with open(log_filepath, "w", encoding="utf-8") as log_file:
            log_file.write("--- PLANAR TRACKING WITH HARMONIZATION RUN LOG STARTED ---\n")
            try:
                while True:
                    ret, frame = cap.read()
                    if not ret:
...
if __name__ == "__main__":
    ...
    try:
        # Harmonization blending for natural color integration
        tracker = PlanarARTrackerHarmonization(ad_image_path=LOGO_ASSET)
        tracker.start_pipeline(video_path=TARGET_VIDEO)
