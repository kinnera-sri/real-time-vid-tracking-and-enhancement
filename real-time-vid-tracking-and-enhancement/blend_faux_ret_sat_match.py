...
# ============================================================
# RETINEX + HARMONIZATION BLENDING
# ============================================================

class RetinexHarmonizationBlender:
    def __init__(self, ad_image_path: str, feature_limit: int = 2000):
        """Initializes the tracker with SIFT, FLANN matching, Retinex and harmonization blending."""
        ...
        self.win_live = "Live Retinex+Harmonization Tracking (Press 'q' to Quit)"
    ...
    def _apply_lab_harmonization(
        ...
        """
        Harmonization focused on matching logo saturation,
        luminance and color statistics to the local background.

        Unlike the previous version:
        - NO saturation protection
        - Saturation is explicitly matched to the background
        ...
        adjusted_lab = blend_lab.copy()

        region = adjusted_lab[mask_region]
        ...
        adjusted_lab[mask_region] = region

        adjusted_lab = np.clip(
            adjusted_lab,
            0,
            255
        ).astype(np.uint8)
        ...
        logo_s = harmonized_hsv[:, :, 1][mask_region]
        bg_s = frame_hsv[:, :, 1][mask_region]

        logo_mean = np.mean(logo_s)
        logo_std = np.std(logo_s)

        bg_mean = np.mean(bg_s)
        bg_std = np.std(bg_s)

        matched_sat = (
            (logo_s - logo_mean)
            * (bg_std / (logo_std + 1e-6))
            + bg_mean
        )

        harmonized_hsv[:, :, 1][mask_region] = np.clip(
            matched_sat,
            0,
            255
        )
        ...
        harmonized = cv2.cvtColor(
            harmonized_hsv.astype(np.uint8),
            cv2.COLOR_HSV2BGR
        )

        return harmonized

    def blend_ad_into_roi(self, frame: np.ndarray, dst_pts: np.ndarray, feather_radius: int = 5) -> np.ndarray:
        """
        Combined Retinex + Harmonization blending pipeline:
        1. Multi-scale Retinex for accurate lighting
        2. Feathered edge smoothing for seamless blending
        3. Alpha blending with soft mask
        4. LAB harmonization for color matching
        """
        ...
        # Apply LAB harmonization for color matching
    ...
    def track_and_render_frame(self, img1: np.ndarray, frame_idx: int, log_file_handle) -> np.ndarray:
        ...
        # Blend graphic overlays onto the background image using Retinex + Harmonization
    ...
    def start_pipeline(self, video_path: str, target_width: int = 1280):
        ...
        output_dir = "./real-time-vid-tracking-and-enhancement/tracked_frames_ret_sat_match"
        os.makedirs(output_dir, exist_ok=True)
        log_filepath = "./real-time-vid-tracking-and-enhancement/tracking_log_ret_sat_match.txt"
        ...
        with open(log_filepath, "w", encoding="utf-8") as log_file:
            log_file.write("--- RETINEX + HARMONIZATION BLENDING TRACKING RUN LOG STARTED ---\n")
            try:
                while True:
                    ret, frame = cap.read()
                    if not ret:
...
if __name__ == "__main__":
    ...
    try:
        # Combined Retinex + Harmonization blending for best of both worlds
        blender = RetinexHarmonizationBlender(ad_image_path=LOGO_ASSET)
