...
def blend_ad_into_roi(frame, ad_img, dst_pts, mode='seamless_feather'):
    """Blend `ad_img` into `frame` at polygon `dst_pts`.

    mode: one of 'seamless', 'feather', 'seamless_feather'
    - 'seamless': use cv2.seamlessClone only (binary mask)
    - 'feather': use soft alpha feather blending only
    - 'seamless_feather' (default): try seamlessClone then apply feathered alpha for smooth border
    """
    warped_ad, warped_mask = create_warped_ad_and_mask(ad_img, frame.shape, dst_pts)
    if np.count_nonzero(warped_mask) == 0:
        return frame

    # Compute an integer center for seamlessClone (requires mask to be 8-bit binary)
    center = tuple(np.mean(dst_pts, axis=0).astype(int))

    # Prepare a binary mask for seamlessClone (threshold the warped_mask)
    mask_for_clone = cv2.threshold(warped_mask, 127, 255, cv2.THRESH_BINARY)[1].astype(np.uint8)

    blended_base = frame.copy()

    # Seamless cloning stage (optional)
    if mode in ('seamless', 'seamless_feather'):
        try:
            if np.count_nonzero(mask_for_clone) > 0:
                blended_base = cv2.seamlessClone(warped_ad, frame, mask_for_clone, center, cv2.MIXED_CLONE)
        except cv2.error:
            blended_base = frame.copy()

    # Feather (soft alpha) blending stage (optional)
    if mode in ('feather', 'seamless_feather'):
        ...
        src_f = warped_ad.astype(np.float32)
        dst_f = blended_base.astype(np.float32)

        composite = (alpha_3c * src_f) + ((1.0 - alpha_3c) * dst_f)
        composite = np.clip(composite, 0, 255).astype(np.uint8)

        # Merge composite into the original frame using binary mask to preserve outside pixels
        outside_mask = cv2.bitwise_not(mask_for_clone)
        outside = cv2.bitwise_and(frame, frame, mask=outside_mask)
        inside = cv2.bitwise_and(composite, composite, mask=mask_for_clone)
        final = cv2.add(outside, inside)
        return final

    # If mode was 'seamless' only, return the seamless result
    return blended_base
...
def run_pure_opencv_tracker(video_path):
    ...
    index_params: Mapping[str, object] = {"algorithm": FLANN_INDEX_KDTREE, "trees": 5}
    search_params: Mapping[str, object] = {"checks": 50}
